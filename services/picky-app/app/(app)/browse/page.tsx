    ...
    return (
        <section className="px-8 sm:px-16">
            ...
            <Carousel opts={{ align: "center", dragFree: true, loop: true }}>
                <CarouselContent className="py-2" viewportClassName="[mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
                    {dishes.map((dish) => (
                        <CarouselItem key={dish.id} className="basis-3/5 sm:basis-2/5 md:basis-1/3 lg:basis-1/4">
                            <DishCard dish={dish} />
                        </CarouselItem>
                    ))}
                </CarouselContent>
